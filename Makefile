...
run:
	...
	GAMEHUB_INBOUND_RETRY_AFTER=$(GAMEHUB_INBOUND_RETRY_AFTER) \
	GAMEHUB_INBOUND_BUCKET_MAX_STALE=$(GAMEHUB_INBOUND_BUCKET_MAX_STALE) \
	GAMEHUB_INBOUND_BUCKET_EVICT_THRESHOLD=$(GAMEHUB_INBOUND_BUCKET_EVICT_THRESHOLD) \
...
lint:
	docker run --rm --memory="2g" -v "$$(pwd):/app" -w /app golangci/golangci-lint:latest sh -c "go mod download && golangci-lint run --timeout=5m"
...
stress-demo-docker:
	...
	docker run -d \
		...
		-e GAMEHUB_INBOUND_RETRY_AFTER=$(GAMEHUB_INBOUND_RETRY_AFTER) \
		-e GAMEHUB_INBOUND_BUCKET_MAX_STALE=$(GAMEHUB_INBOUND_BUCKET_MAX_STALE) \
		-e GAMEHUB_INBOUND_BUCKET_EVICT_THRESHOLD=$(GAMEHUB_INBOUND_BUCKET_EVICT_THRESHOLD) \
